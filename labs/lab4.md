...
> ### _Задание_:
>
> Удалите ранее созданные переменные с вашей фамилией и выбранным названием города. Попробуйте вывести их значение на экран. Скриншот вывода поместите в
...
Значение переменной `$PATH` вашей командной оболочки может отличаться в случае использования команды `su` вместо команды `su -`, так как последняя команда позволяет дополнительно использовать значения переменных окружения целевого пользователя.

> ### _Задание_:
>
> 1. Выполните команду `su`.
> 2. Выведите на экран содержимое переменной окружения `$PATH`.
> 3. Выполните команду `exit`.
> 4. Выполните команду `su -`.
> 5. Выведите на экран содержимое переменной окружения `$PATH`.
> 6. Выполните команду `exit`.
> 7. В чем отличие содержимого переменной окружения `$PATH` разных режимов доступа? Ответ и скриншоты вывода пунктов 2 и 5 поместите в отчет.
...
Вы можете использовать команду `env` для установки значения переменной `$LANG` или любой другой переменной окружения одного экземпляра командной оболочки bash в рамках одной команды. Приведенные ниже команды демонстрируют влияние значения переменной `$LANG` на работу механизма поиска файлов по шаблонам (данный механизм будет рассмотрен в последующих лабораторных работах). Выполните данный пример и ознакомьтесь с выводом.

```
env LANG=C bash -c 'ls File[a-z]'
env LANG=en_US.UTF-8 bash -c 'ls File[a-z]'
```
